...
class Graph_learner(nn.Module):
    ...
    def forward(self, inputs):  
        ...
        probs = torch.matmul(Xq, Xk.transpose(-1, -2))  
        return probs
...
class Decoder_Pre(nn.Module):
    ...
    def forward(self, num_of_task, dec_out, z, dec_self_mask=None, dec_enc_mask=None):
        ...
        z1 = self.fc_in2[num_of_task](z1)
        z1=self.activation(z1)
        dec_pred_out = self.decoder_pre[num_of_task](dec_out, z1, x_mask=dec_self_mask, cross_mask=dec_enc_mask)
        pred_out = self.fc_out[num_of_task](dec_pred_out)
        return pred_out  
...
class GATLayer(nn.Module):
    ...
    def forward(self, h, adj,batch_i):
        ...
        a_input = torch.cat([Wh.repeat(1, N).view(N * N, C), Wh.repeat(N, 1)], dim=1).view(N, N, 2 * self.output_feature)  # [B,N,N,2C]
...
class GAT(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_of_task, dropout, alpha, nheads, concat=True):
        ...
        self.dropout = dropout
        self.attention = [GATLayer(input_size, hidden_size, num_of_task, dropout=dropout, alpha=alpha, concat=True) for _ in range(nheads)]
...
class VAE(nn.Module):
    def __init__(self, enc_spatio, enc_temporal, sampling_z, GAT, dec_pre, dec_rec, T, LATENT_CODE_SIZE, enc_in, k, x0,d_model=64, topk_indices_ji=None, dropout=0.2, embed='fixed', freq='t'):
        ...
        self.GAT = GAT
        self.fc = nn.Linear(LATENT_CODE_SIZE, T * d_model)
        self.T = T
        self.LATENT_CODE_SIZE = LATENT_CODE_SIZE
        self.d_model = d_model
        self.temperature = 0.01
        self.grelen_enc_spatio = Graph_learner(T, n_hid=128, n_head_dim=128, head=2)
        self.linear1 = nn.Linear(1, d_model)  
        self.grelen_head = 2
        self.device = device

        self.enc_embedding0 = DataEmbedding(enc_in, d_model, embed, freq, dropout)#freq并未用到
    ...
    def forward(self, all_input, all_target, batch_i, config, flag, step, device):
        ...
        inputs = all_input
        B = batch_size
        probs = self.grelen_enc_spatio(inputs) 
        mask_loc = torch.eye(NUM_OF_TASK, dtype=bool).to(self.device)
        probs_reshaped = probs.masked_select(~mask_loc).view(B, self.grelen_head, NUM_OF_TASK * (NUM_OF_TASK - 1)).to(self.device)
        ...
        adj_list = torch.ones(self.grelen_head, B, NUM_OF_TASK, NUM_OF_TASK).to(self.device)
        mask = ~torch.eye(NUM_OF_TASK, dtype=bool).unsqueeze(0).unsqueeze(0).to(self.device)
        mask = mask.repeat(self.grelen_head, B, 1, 1).to(self.device)
        ...
        for num_of_task in range(NUM_OF_TASK):
            ...
            src = src.float()
            enc_inp = self.enc_embedding0(src) + self.enc_embedding(src)
